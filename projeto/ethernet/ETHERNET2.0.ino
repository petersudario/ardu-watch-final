...
byte servidor[] = {192, 168, 0, 8};                // aqui deve ser colocado o endereço ip do servidor (seu endereço ipv4)
...
void loop()
{
  ...
  if (client)
  { // SE EXISTE CLIENTE FAZ
    while (client.connected())
    { // ENQUANTO EXISTIR CLIENTE CONECTADO, FAZ
      if (client.available())
      {                                // SE O CLIENTE ESTÁ HABILITADO, FAZ
        ...
        { // SE ENCONTRAR "\n" É O FINAL DO CABEÇALHO DA REQUISIÇÃO HTTP, FAZ
          ...
          { // SE ENCONTRAR O CARACTER "?", FAZ
            ...
            {                             // SE ENCONTRAR O PARÂMETRO "ledParam=1", FAZ
          ...
          if (client.connect(servidor, server(80)))
          {
            client.print("GET /projeto/salvarGps.php?");
            client.print("GET /projeto/salvarRtc.php?");
            client.print("GET /projeto/salvarOximetro.php?");
            client.print("gx=");
            client.print();   //Aqui vai a variavel onde nesse caso, foi usada pelo GPS. 
            client.print( 	  )
